...
class Koudai:
    ...
    def __init__(self):
        ...
        if res['status'] == 200:
            ...
            if not self.pref.tokenverify():
                WARN('token失效，尝试获取新token')
                self.pref.requesttoken()
        else:
    ...
    def getMainpage(self):
        ...
        header = {
            ...
            'token': self.pref.gettoken()
        }
        form = {
            "lastTime": 0,
            "limit": 10,
            "chatType": 0,
            "roomId": int(self.pref.getroomid())
        }
        res = requests.post(
            url,
    ...
    def getCfgTime13(self):
        t = self.pref.getkdmsgtime13()
        return int(t)

    # 写配置文件时间
    def writeCfgTime13(self, t):
        self.pref.writekdmsgtime13(t)
